...
public class JavaBeanTester {
	 ...
	 * @throws IntrospectionException thrown if the Introspector.getBeanInfo() method throws this exception for the class
	 *                                under test
	 */
	public static <T> void test ( final Class<T> clazz, final String packages, final String... skipThese ) throws
		...
		for ( PropertyDescriptor prop : props ) {
			...
			if ( getter != null && setter != null ) {
				...
				if ( returnType.getCanonicalName().startsWith( packages ) ) {
					test( returnType, packages, skipThese );
				}
				...
				if ( params.length == 1 && params[ 0 ] == returnType ) {
					...
					try {
						...
						T bean = null;
						if ( clazz.getCanonicalName().startsWith( "java.math.BigInteger" ) ) {
							bean = clazz.getConstructor( String.class, Integer.class ).newInstance( "1", 10 );
						} else {
						...
						if ( !( actualValue.getClass().isArray() && expectedValue.getClass().isArray() ) ) {
							assertEquals( expectedValue, actualValue, String.format( "Failed while testing property %s", prop.getName() ) );
						}

						assertTrue( bean.toString() == null || !bean.toString().isEmpty() );
	...
	private static Object buildValue ( Class<?> clazz ) throws
